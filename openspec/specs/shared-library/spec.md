...
把 `eeg_alg` 算法库构建为 Linux 共享库（`libeeg_alg.so`），通过跨平台导出宏 `EEG_ALG_API` 与隐藏可见性（`-fvisibility=hidden`）控制导出符号，使算法库可被外部 C++ 进程动态链接调用；同时埋入稳定的 `extern "C"` 入口 `eeg_alg_abi_version`，作为后续 Windows C# P/Invoke 交付的种子。
